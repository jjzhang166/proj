...
void LoginServer::OnDisconnect( IoConnectBase* connect )
{
	...
	if (login_hlp)
	{
		list_ctrl->SetItemText(login_hlp->item_, kSubitemRunState, _T("µÈ´ýÆô¶¯ÓÎÏ·"));
...
bool LoginServer::OnConnected( IoConnectBase* connect )
{
	...
	if ( tab_option && tab_option->IsLoginSucceed() )
	{
		/*SetIoTimer(5000, [=]( std::size_t ){
			...
			if ( !list_ctrl )
				goto LABLE_ERR;
			if ( !list_ctrl->FindLoginHelperBySession(session.get()) )
				goto LABLE_ERR;
			return / *success* /;
LABLE_ERR:
			session->Disconnect();
...
LoginS::LoginS( Server& server ) : Session(server), json_echo_(*GLuaVM_S::GetInstance(), *this), login_oper_(*this)
{
...
bool LoginS::OnUserMsg( P_MsgNT msg_num, void* param, size_t param_size )
{
	switch (msg_num)
	{
...
bool LoginS::OnBindLoginPid( P_Exec_BindLoginPid& msg )
{
	...
	json_echo_.MemFuncCall(getter, g_call_fn_name_set_accout_wpd, &JsonEcho::MakeParams(account, password));
	const auto& singleton_cfg = stSingletonCfg::GetMe();
	CPath game_path(singleton_cfg.game_full_file_path_);
	game_path.RemoveFileSpec();
	std::string game_patha = CT2CA(game_path.m_strPath);
	json_echo_.MemFuncCall(getter, "SetGamePath", &JsonEcho::MakeParams(game_patha));
	/*{
		POINT pt;
		const auto& cfg = list_ctrl->GetAccountCfg(account.c_str());
		const char* kServiceName = cfg.game_service_.c_str();
	...
	//ÉèÖÃÕýÔÚµÇÂ½
	list_ctrl->SetItemText(login_helper->item_, kSubitemRunState, _T("ÕýÔÚµÇÂ½"));
...
LoginOperS::LoginOperS( LoginS& login_s ) : login_s_(login_s), JsonEchoBase(*GLuaVM_S::GetInstance(), login_s, kObjId_LoginOper)
{
...
bool LoginOperS::AcOrPwdErr()
{
	...
	/*list_ctrl->SetItemText(login_hlp->item_, kSubitemRunState, _T("ÕÊºÅ»òÃÜÂë´íÎó"));
	list_ctrl->SetCheck(login_hlp->item_, FALSE);
	list_ctrl->EraseLoginHelper(login_hlp);
	list_ctrl->DoLoginOne();
	login_hlp = nullptr;*/

	list_ctrl->SetItemText(login_hlp->item_, kSubitemRunState, _T("ÃÜÂë´íÎó"));
	list_ctrl->SetCheck(login_hlp->item_, FALSE);
	list_ctrl->EraseLoginHelper(login_hlp);
	login_hlp = nullptr;
	list_ctrl->DoLoginOne();
...
bool LoginOperS::AcFreeze()
{
	...
	list_ctrl->SetItemText(login_hlp->item_, kSubitemRunState, _T("ÕÊºÅ¶³½á"));
	list_ctrl->SetCheck(login_hlp->item_, FALSE);
...
void LoginOperS::LoginUnRespond()
{
	LOG_O(Log_debug) << "µÇÂ½Æ÷³¤Ê±¼äÎ´ÏìÓ¦";
	auto list_ctrl = PageAccount_List::GetInstance();
	if (!list_ctrl)
		return;
	auto login_hlp = list_ctrl->FindLoginHelperBySession(&login_s_);
	...
	auto item = login_hlp->item_;
	list_ctrl->SetItemText(item, kSubitemRunState, _T("Î´ÏìÓ¦"));
