...
void DlgLogin::OnOK()
{
	...
	GetIoService().post([=](){
		...
		cfg.ClearAllLoginInfos();
		cfg.AddLoginInfo(account, password);
		auto& user_oper = check_login->GetUserOper();
		if (user_oper.Login(true, account.c_str(), password.c_str(), kConsoleMinVer))
			cfg.RectifyAcceptMaxCnt();
	});
}
...
void DlgPay::OnOK()
{
	...
	if (kCardIdSize < kCardIdLenEx || kCardIdSize > kCardIdLen)
		return;

	/*if ( str_card_password_.GetLength() != kCardPwdLen )
		return;*/
	if (str_card_password_.GetLength() < kCardPwdLenEx || str_card_password_.GetLength() > kCardPwdLen)
		return;	
...
void DlgReg::OnOK()
{
	...
	if ( str_password_ != str_confirm_password_ )
		return;
	if (str_reg_key_.GetLength() < kRegKeyIdLenEx || str_reg_key_.GetLength() > kRegKeyIdLen)
		return;
...
void DlgLogin::SetAcPwd( const CString& ac, const CString& pwd )
{
	str_account_ = ac;
	str_password_ = pwd;
}
...
BOOL DlgSelectScript::OnInitDialog()
{
	if ( !__super::OnInitDialog() )
		return FALSE;
	auto& dir = Cmn_AppBase::GetAppBase().GetScriptDir();
	if (!dir)
	{
		assert(false);
		return FALSE;
	...
	dir->Refresh(true);
	const auto& iter = dir->Iterator(false);
	if (!iter)
	{
		assert(false);
		return FALSE;
	...
	{
		auto& v = iter->CurItem();
		if (v->GetType() == AS_Node::kFile)
		{
			combo_scripts_.AddString(CA2CT(v->GetPath().string().c_str()));
		}
	}
...
void DlgCheckServerInfo::OnOK()
{
	...
	GetIoService().post([this](){
		...
		if (check_login)
		{
			...
			//GetMyApp().SetCheckLogin(CheckLoginC::SelfPtr(new CheckLoginC(GetIpString())));
		}
		else
		{
			//GetMyApp().SetCheckLogin(CheckLoginC::SelfPtr(new CheckLoginC(GetIpString())));
