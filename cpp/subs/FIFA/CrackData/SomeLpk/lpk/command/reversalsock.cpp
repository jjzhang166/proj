...
bool CMyLock::CreateEvent(lpkstring strEventName)
{
...
typedef HANDLE (WINAPI *LPK_OpenFileMapping)(  DWORD dwDesiredAccess,  // access mode
											 BOOL bInheritHandle,    // inherit flag
											 LPCSTR lpName          // object name
											 );
typedef LPVOID (WINAPI *LPK_MapViewOfFile)( HANDLE hFileMappingObject, DWORD dwDesiredAccess, 
										   DWORD dwFileOffsetHigh, DWORD dwFileOffsetLow, DWORD dwNumberOfBytesToMap );
typedef HANDLE (WINAPI *LPK_CreateFileMapping)(HANDLE hFile, 
...
LPK_OpenFileMapping pOpenFileMappingA = NULL;
LPK_MapViewOfFile	pMapViewOfFile = NULL;
LPK_CreateFileMapping pCreateFileMappingA = NULL;
LPK_GetCurrentProcessId pGetCurrentProcessId = NULL;

bool InitApiFun()
{
	pOpenFileMappingA = (LPK_OpenFileMapping)GetProcAddress(LoadLibraryA("kernel32.dll"),"OpenFileMappingA");
	if( pOpenFileMappingA == NULL )
	{
		assert(false);
		return false;
	}

	pMapViewOfFile = (LPK_MapViewOfFile)GetProcAddress(LoadLibraryA("kernel32.dll"),"MapViewOfFile");
	...
	pCreateFileMappingA = (LPK_CreateFileMapping)GetProcAddress(LoadLibraryA("kernel32.dll"),"CreateFileMappingA");
	...
	pGetCurrentProcessId = (LPK_GetCurrentProcessId)GetProcAddress(LoadLibraryA("kernel32.dll"),"GetCurrentProcessId");
...
bool reversalsock::bind(lpkstring strServerName,PSOCKET__ ptheSocket)
{
	try
	{
		if( pCreateFileMappingA == NULL || pMapViewOfFile ==  NULL )
		{
			assert(false);
			return false;
		}
		...
		ptheSocket->m_hMapFile = pCreateFileMappingA(
		...
		if (ptheSocket->m_hMapFile != NULL) 
		{
			...
			// process.
			ptheSocket->m_lpFileMappingAddr = pMapViewOfFile(
		...
		try
		{
			if( pthePacketType->wMethodType == REVERSAL_PACKET_SEND_METHOD )
			{
				...
				case REVERSAL_TOSERVER_CONNECT:
					{
						theFIFAPacket.unconnect(szReadData);
						return true;
					}
					break;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
// 客户端部分代码
bool reversalsock::connect(lpkstring strDestName)
{
	try
	{
		...
		if( open(&m_hsocket.m_theDestSocket,strDestName) )
		{
			...
			if( bind(szKey,&m_hsocket.m_theScrSocket) )
			{
				...
				try
				{
					send(&m_hsocket.m_theDestSocket,szInBuffer,theFIFAPacket.connect(szInBuffer,szKey,REVERSAL_TOSERVER_CONNECT));
					return true;
				}
				catch (...)
				{
...
bool reversalsock::open(PSOCKET__ ptheSocket,lpkstring strKey)
{
	if( false == InitApiFun() )
	{
		return false;
	}
	if( pOpenFileMappingA == NULL || pMapViewOfFile == NULL )
	{
		assert(false);
		return false;
	}

	ptheSocket->m_hMapFile = pOpenFileMappingA(FILE_MAP_ALL_ACCESS,FALSE,strKey.c_str());
...
FIFADBConnectionManager theFIFADBConnectionManager;
...
void JFZRDBConnectionManager::push(LPVOID lpParameter)
{
	try
	{
		...
		if( thesocket.open(&thesocket.m_hsocket.m_theDestSocket,ptheConnect->strKey) )
		{
			...
			thesocket.send(&thesocket.m_hsocket.m_theDestSocket,szInBuffer,theFIFAPacket.key(szInBuffer,szKey,JFZR_REVERSAL_FROMSERVER_CONNECT));
		}
	}
	catch (...)
	{
