...
public:
	...
	void UnLock();
	bool IsLock();
	bool CreateEvent(std::string strEventName);
	void SetEvent_();
	void ResetEvent_();
	void close()
	{
...
{
	HANDLE			m_hMapFile;
	std::string		m_strFileMappingName;
...
public:
	...
	~reversalsock(void);
public:
	bool open(PSOCKET__ ptheSocket,std::string strKey);
	bool read(PSOCKET_ ptheSocket, char* pOutBuffer);
	bool send(PSOCKET__ ptheSocket, LPVOID lpBuffer, DWORD dwLen);

	bool bind(std::string strServerName,PSOCKET__ ptheSocket);
	bool doconnect();

	bool connect(std::string strDestName);
	void close();
};
