...
//#define CRC32(buffer,size)  crc32(-1,buffer,size)
...
unsigned int GetCRC32(const unsigned char *buf,int size);



#define CRC32_INLINE(pscr,len,ret)int i, crc;\
	const unsigned char *buf=pscr;\
	int size=len;\
	crc = 0xFFFFFFFF;\
	for (i = 0; i < size; i++)\
...
#define DWORD_XOR_VALUE			0xD8D8AB73		// 异或，用于加密的key值