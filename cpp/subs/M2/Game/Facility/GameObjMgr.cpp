...
GameObjBase* GameObjMgrBase::CreateGameObj(GameObjType type) const
{
	switch (type)
	{
	case kGOT_Invalid:
...
GameObjBasePtrT GameObjMgrBase::FindObjInCircleByType(GameObjType obj_type,const stCD_VecInt& center, int radius) const
{
	const auto& form = MakeCircleStdForm(center, radius);
	for (auto& v : objs_)
	{
