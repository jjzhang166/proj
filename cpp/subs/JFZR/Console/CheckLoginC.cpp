...
bool CheckLoginC::OnConnected( IoConnectBase* connect )
{
	...
	LOG_ENTER_FUNC;
	LOG_O(Log_trace) << "目标IP->" << kDstIp << "\treak ip->" << kRemoteCheckServerIp;
	...
	{
		LOG_ENTER_FUNC;
		LOG_O(Log_debug) << "check server ip check failed";
		assert(false);
...
void CheckLoginOperC::Login( const std::string& user_name, const std::string& user_pwd )
{
	...
	if (res)
	{
		...
		//检测目标IP
#ifndef _DEBUG
		const char* const kUuidGenerator = "*()fdja";
		const auto& kDstIp = check_login_.socket_.remote_endpoint().address().to_string();
		const auto& kUuidRealDstIp = GenUuidStrFromGenerator(kDstIp, kUuidGenerator);
		LOG_ENTER_FUNC;
		LOG_O(Log_trace) << "目标IP->" << kDstIp << "\treak ip->" << kRemoteCheckServerIp;
		const auto& kUuidDstIp = GenUuidStrFromGenerator(VMP_PROTECT_STR(kRemoteCheckServerIp), kUuidGenerator);
		if ( kUuidDstIp != kUuidRealDstIp )
		{
			...
			LOG_O(Log_debug) << "check server ip check failed";
			SetIoTimer(20 * 1000, []( IoTimer* ){
				throw CmnExitIoThreadException("95136");
			});
		}
#endif
