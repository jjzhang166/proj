...
extern void AddDoorLinkInfo( const stFbDoorLink& door_link );
...
void DxPressKey(DWORD dwVk,DWORD dwFlags);		// dx按键
int GetDecryValue( DWORD dwAddr, int def_ret = 0 );	// 解密
void EncryptionValue_int(DWORD dwValueAddr,int nValue);	// 加密
int GetSellItemPrice(DWORD dwItemMuBanAddr);		// 获取物品出售价钱

extern int g_nHitCount;
//void MMO_HookHit();		// 开启多倍攻击
void MMO_UnHookHit();	// 关闭多倍攻击，恢复hook
void MMO_HookMonsterHit();		// 开启无敌
void MMO_UnHookMonsterHit();	// 关闭无敌，恢复hook
void MMO_Render(DWORD dwSleepTime);		// cpu优化
...
float GetMapCoorDinates_Z();		// 获取当前角色所在坐标的地图z坐标
...
// 2王立一星和2星  3 王立三星和四星
DWORD GetKingQuestInfoAddr(int nKingQuestIndex = 2);

DWORD TestNopCheck();//测试过的一些没必要的东西
//是否开启无敌
void EnableWuDi( bool enable_wu_di );

#ifdef _USE_SELF_DIK_MACRO_
// dx按键
