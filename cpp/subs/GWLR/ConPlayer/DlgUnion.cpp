...
BOOL DlgLogin::OnInitDialog()
{
	if (str_account_.IsEmpty())
	{
		...
		str_account_ = CA2CT(singleton_cfg.ac_.c_str());
		str_password_ = CA2CT(singleton_cfg.pwd_.c_str());
	}
...
void DlgLogin::OnClose()
{
	...
	singleton_cfg.ac_ = CT2CA(str_account_);
	singleton_cfg.pwd_ = CT2CA(str_password_);
	singleton_cfg.Save();
	__super::OnClose();
}
