...
bool FuncObj_NormalS::OnGameProccessInfo( const P_Exec_GameProccessInfo& game_proccess_info )
{
	...
	if (page_ac->GetSession())
		return false;
	page_ac->SetSession(this);
	return true;
}
...
void FuncObj_NormalS::DoSomeConnected()
{
	...
	GetSessionOper().ThisMemCall(nullptr, "SetMultiHit", &JsonEcho::MakeParams(sin_cfg.multi_hit_));
	VMP_END;
}
...
void FuncObj_NormalS::OnDisconnect( IoConnectBase* connect )
{
	...
	if ( this == page_ac->GetSession() )
		page_ac->SetSession(nullptr);
}
