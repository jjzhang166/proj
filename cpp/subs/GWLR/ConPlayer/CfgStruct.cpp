...
{
	multi_hit_ = kMultiHitRate_Min;
}

SingleProfile& SingleProfile::GetMe()
{
	struct stHelper{
		stHelper(){
			auto str_path = GetModulePath((LPCTSTR)nullptr);
			str_path = str_path + _T("app.ini");
			std::string str_path_a = CT2CA(str_path);
			the_obj.SetProfile(str_path_a);
		}
		SingleProfile		the_obj;
	};
	static stHelper hlp;
	return hlp.the_obj;
}

bool SingleProfile::Load()
{
	Read(kMainSetion, "游戏路径", game_path_, game_path_);
	Read(kMainSetion, "倍攻", multi_hit_, multi_hit_);
	Read(kMainSetion, "帐号", ac_, ac_);
	Read(kMainSetion, "密码", pwd_, pwd_);
	return true;
}

bool SingleProfile::Save()
{
	Write(kMainSetion, "游戏路径", game_path_);
	Write(kMainSetion, "倍攻", multi_hit_);
	Write(kMainSetion, "帐号", ac_);
	Write(kMainSetion, "密码", pwd_);
	return true;
}

const char* const SingleProfile::kMainSetion = "主要配置";
