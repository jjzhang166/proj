...
局部变量 接任务信息 = 自定义参数[1]
接任务信息[1] 保存了要接的任务所在的村庄名；接任务信息[2] 保存了该任务所属的NPC名

局部变量 提交任务信息 = 自定义参数[3]
提交任务信息[1] 保存了提交任务所在的村庄名；提交任务信息[2] 保存了提交该任务所属的NPC名
]]
定义函数 接_通用的接任务处理( 任务ID, 任务名, 自定义参数 )
	...
	如果 没有 打开某个城镇的某个NPC(接任务信息[1], 接任务信息[2], 接任务信息[3], 接任务信息[4]) 那么
		LuaLogE("接任务时，打开某个城镇的某个NPC失败，任务名：" .. 任务名)
		返回 假
	结束
...
定义函数 提交_通用的提交任务处理( 任务ID, 任务名, 自定义参数 )
	...
	如果 没有 打开某个城镇的某个NPC(提交任务信息[1], 提交任务信息[2], 提交任务信息[3], 提交任务信息[4]) 那么
		LuaLogE("提交任务时，打开某个城镇的某个NPC失败，任务名：" .. 任务名)
		返回 假
	结束
...
定义函数 通用_发送邮件( 邮寄的物品, 金币, 邮寄的角色名 )
	--添加好友(邮寄的角色名)
	返回 发送邮件(邮寄的物品, 金币, 邮寄的角色名, "米拉德村", "信差艾露秋梨欧")
结束
...
定义函数 通用_做悬赏任务( 赏金任务类型, 城镇名, 看板, 赏金目标副本黑名单 )
	...
	局部变量 副本ID = 获取任务所做副本ID(任务ID)
	如果 副本ID == 空 那么
		LuaLogE("通用_做悬赏任务之获取任务所做的副本名失败，任务名：" .. 获取任务名(任务ID));
		放弃指定ID的任务(任务ID)
		返回 假
	结束
...
定义函数 通用_打一个指定的王立精英副本( 副本名, 打副本函数 )
	如果 没有 移动到指定的王立精英副本(副本名) 那么
		返回 假
	结束
	如果 打副本函数 == 空 那么
		打副本函数 = 通用_打死副本BOSS
	...
	for 索引, 副本信息 in ipairs(所有副本信息) 执行
		如果 索引 >= 副本索引 那么
			...
			如果 打副本函数 ~= 空 那么
				...
				打副本函数 = 定义函数 () 打副本函数_(unpack(参数信息)) 结束
			结束
			局部变量 成功, 副本ID = 通用_打一个指定的王立精英副本(副本信息[1], 打副本函数)
			如果 成功 == 真 那么
				返回 成功, 副本ID, 索引 + 1
			结束
			如果 成功 == 假 那么
