...
function 中文角色名_设置前缀字符出现几率( rate_pre )
	if type(rate_pre) ~= "number" then
		LuaLogE("中文角色名_设置前缀字符出现几率之几率参数必须是数字类型")
		return
	end
	RunInfo("中文角色名_设置前缀字符出现几率")
	return scmd():RNM_SetOccurRate_Pre(rate_pre)
end

function 中文角色名_设置间隔字符出现几率( rate_middle )
	if type(rate_middle) ~= "number" then
		LuaLogE("中文角色名_设置间隔字符出现几率之几率参数必须是数字类型")
		return
	end
	RunInfo("中文角色名_设置间隔字符出现几率")
	return scmd():RNM_SetOccurRate_Middle(rate_middle)
end

function 中文角色名_设置后缀字符出现几率( rate_suffix )
	if type(rate_suffix) ~= "number" then
		LuaLogE("中文角色名_设置后缀字符出现几率之几率参数必须是数字类型")
		return
	end
	RunInfo("中文角色名_设置后缀字符出现几率")
	return scmd():RNM_SetOccurRate_Suffix(rate_suffix)
end

function 中文角色名_添加前缀字符串( str_some_pre )
	if type(str_some_pre) ~= "string" then
		LuaLogE("中文角色名_添加前缀字符串之字符串参数必须是字符串类型")
		return
	end
	RunInfo("中文角色名_添加前缀字符串")
	return scmd():RNM_AddSomeStr_Pre(str_some_pre)
end

function 中文角色名_添加间隔字符串( str_some_middle )
	if type(str_some_middle) ~= "string" then
		LuaLogE("中文角色名_添加间隔字符串之字符串参数必须是字符串类型")
		return
	end
	RunInfo("中文角色名_添加间隔字符串")
	return scmd():RNM_AddSomeStr_Middle(str_some_middle)
end

function 中文角色名_添加后缀字符串( str_some_suffix )
	if type(str_some_suffix) ~= "string" then
		LuaLogE("中文角色名_添加后缀字符串之字符串参数必须是字符串类型")
		return
	end
	RunInfo("中文角色名_添加后缀字符串")
	return scmd():RNM_AddSomeStr_Suffix(str_some_suffix)
end

function 中文角色名_添加随机字符串( str_some_random )
	if type(str_some_random) ~= "string" then
		LuaLogE("中文角色名_添加随机字符串之字符串参数必须是字符串类型")
		return
	end
	RunInfo("中文角色名_添加随机字符串")
	return scmd():RNM_AddSomeRandomStr(str_some_random)
end

function 中文角色名_清空所有数据()
	RunInfo("中文角色名_清空所有数据")
	return scmd():RNM_ClearAllData()
end
