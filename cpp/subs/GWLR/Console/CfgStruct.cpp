...
const stSpecificCfg::stRoleInfo* stSpecificCfg::FindFirstFatigueValidRole()
{
	...
	if ( time_now <= theApp.fatigue_end_to_clear_time_ )
	{
		for ( auto& role : roles_info_ )
		{
			...
			else
			{
				...
				if ( (theApp.fatigue_end_to_clear_time_ - role.the_time_).totalHours() >= 24 )
				{
	...
	else
	{
		for ( auto& role : roles_info_ )
		{
			...
			{
				if ( role.the_time_ <= theApp.fatigue_end_to_clear_time_ )
				{
					role.ResetFatigueData();
					return &role;
				}
...
{
	return (int)roles_info_.size() < stSingletonCfg::GetMe().role_max_cnt_;
}

void stSpecificCfg::SetRoleFatigueData( const std::string& role_name, unsigned __int64 fatigue_data )
{
...
stSpecificCfg::stRoleInfo::stRoleInfo( const std::string& role_name, unsigned __int64 fatigule_data )
{
...
void stSpecificCfg::stRoleInfo::SetFatigueData( unsigned __int64 fatigue_data )
{
	if ( fatigue_data <= 0 )
	{
		DateTimeLocal time_now;
		if ( time_now <= theApp.fatigue_end_to_clear_time_ )
		{
			if ( (theApp.fatigue_end_to_clear_time_ - the_time_).totalHours() >= 24 )
			{
				RefreshTime();
			}
		}
		...
		{
			if ( the_time_ <= theApp.fatigue_end_to_clear_time_ )
			{
				RefreshTime();
			}
		}		
...
void stSingletonCfg::DoCloseTheSession(const BindAcInfoT& the_clr_ac)
{
	...
	SetIoTimer(1000 * 6, [the_clr_ac]( IoTimer* ){
		...
		/*auto need_close_cnt = page_list->CalcConnectionsCnt() - stSingletonCfg::GetMe().GetAllowMaxLoginCnt();
		for ( auto i = 0; i < need_close_cnt; ++i )
		{
		}*/
