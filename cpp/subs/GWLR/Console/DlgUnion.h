...
class DlgLoginBase : public DlgUseful{
protected:
	DlgLoginBase( UINT id );
...
//dlg login
class DlgLogin : public DlgLoginBase, public Singleton<DlgLogin, Singleton_Instance>{
public:
	DlgLogin();
};

class DlgRemoveBind : public DlgLoginBase, public Singleton<DlgRemoveBind, Singleton_Instance>{
public:
	DlgRemoveBind();
};
...
//留言版
class DlgMsgBoard : public DlgUseful, public Singleton<DlgMsgBoard, Singleton_Instance>{
public:
	DlgMsgBoard();
...
class DlgAddLoginAc;
class IDlgAddLoginSink{
public:
	virtual void OnAdd( DlgAddLoginAc& dlg ) = 0;
};

class DlgAddLoginAc : public DlgUseful{
public:
	DlgAddLoginAc( IDlgAddLoginSink* sink );
...
public:
	CString				ac_;
	CString				pwd_;
	IDlgAddLoginSink*	sink_;
};

class DlgAutoDial : public DlgUseful, public Singleton<DlgAutoDial, Singleton_Instance>{
public:
	DlgAutoDial();
	bool NeedReDial();
...
protected:
	...
	BOOL OnInitDialog();
	virtual void OnOK();
	afx_msg void OnBtIsAutoDial();
	afx_msg void OnSelchangeDialMode();
	DECLARE_MESSAGE_MAP()
...
private:
	CComboBox		combo_select_mode_;

public:
	int				cur_role_use_out_cnt_;
	TimeElapse		timer_for_dial_;
};