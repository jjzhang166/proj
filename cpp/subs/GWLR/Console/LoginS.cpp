...
LoginServer::LoginServer() : Server(P_Port_ForLogin)
{
...
LoginS::LoginS( Server& server ) : Session(server), json_echo_(GLuaVM_S::GetMe(), *this), login_oper_(*this)
{
...
bool LoginS::OnBindLoginPid( P_Exec_BindLoginPid& msg )
{
	...
	P_Exec_Login_AcPwd ac_info = { 0 };
	Cmnx_CpyMem(ac_info.account_, account.c_str(), account.size());
	Cmnx_CpyMem(ac_info.password_, password.c_str(), password.size());
	if ( !UserMsg(P_Exec_Login_AcPwd::value, &ac_info, sizeof(ac_info)) )
	{
		assert(false);
		return false;
	}
...
bool LoginOperS::AcOrPwdErr()
{
	...
	login_hlp = nullptr;
	if (stSingletonCfg::GetMe().ignore_pwd_err_)
	{
		list_ctrl->AddLoginItem(login_item, 0);
	}
	else
	{
		list_ctrl->SetItemText(login_item, kSubitemRunState, _T("密码错误"));
		std::string ac = CT2CA(list_ctrl->GetItemAccount(login_item));
		std::string ser_name = CT2CA(list_ctrl->GetItemServiceName(login_item));
		//添加禁止启动项
		list_ctrl->AddProhibitItem(ac, ser_name, stSpecificCfg::kPR_AcPwdErr);
	}
