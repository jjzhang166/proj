...
enum enListSubItems{
	...
	kSubitemProhibitTime,		//禁止时间
	kSubitemGameAcType,			//号的种类
	kSubitemAutoRunScript,		//自动运行脚本
	kSubitemRoleName,			//角色名
	kSubitemCurRoleIdx,			//角色索引
	kSubitemRoleLevel,			//角色等级
	kSubitemFreeLieShouCnt,		//自由狩猎许可证
	kSubitemSingleExp,			//单倍经验
	kSubitemDoubleExp,			//双倍经验
	kSubitemCurMapRoomName,		//所在地图或房间
	kSubitemCurGold,			//当前金币
	kSubitemGainedGold,			//获得的金币
...
public:
	//开始登陆游戏
	//bool BeginLogin( int item, int microseconds );
	bool AddLoginItemSmart( int item, int microseconds );
	bool AddLoginItem( int item, int microseconds );
	bool AddFirstLoginItem( int item );
	bool CanLoginItem( int item );

public:
	//执行登陆一个帐号
	bool DoLoginOne();
	bool DoLoginOneImpl();
	DWORD CreateTheLoginProcess( LPCTSTR login_file_name, LPCTSTR login_dir );
	bool TestDoLoginOne();
	//得到正在登陆和在线的数量
...
public:
	...
	//禁止时间
	void SetProhibitTime( FuncObj_NormalS& nor, const std::string& time );
...
public:
	...
	//切换优化
	afx_msg void OnMenuSwitchYouHua();
	//批量设置脚本
	afx_msg void OnMenuSetAllScript();
	DECLARE_MESSAGE_MAP()
