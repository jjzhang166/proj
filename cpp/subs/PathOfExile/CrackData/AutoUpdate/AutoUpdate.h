...
#define CheckExeAddr(addr) assert((pt_pcvoid)addr > (pt_pcvoid)exe_code_range_begin_ && (pt_pcvoid)addr < exe_code_range_end_)
...
private:
	...
	//PathOfExileBase
	bool Scan_PathOfExileBase(Cmn_FC_OFstream& ofs);

private:
	Cmn_FeatureCodeScan_Direct	scan_base_;
};
