...
定义函数 处理事件(事件名)
    ...
    否则如果 事件名 == "背包已满事件" 那么
        如果 使用传送卷轴到地图() ~= 真 那么
            ...
            返回 假
        结束
        如果 杀到指定地图("The Sarn Encampment",3,假) ~= 真 那么
            LogE("背包满之杀到城镇失败")
            返回 假
        结束
        如果 自动存仓() ~= 真 那么 
...
定义函数 处理游戏内场景()
    循环 真 执行
        局部变量 当前地图 = 当前地图名字()
        如果 当前地图 ~= "The Eternal Laboratory" 且 当前地图 ~= 获得进行中异界地图() 那么 
            如果 根据类型获得物品数量(物品类型.异界地图) <= 0 那么
                ...
                结束
            结束
            如果 杀到指定地图("The Eternal Laboratory",3) ~= 真 那么
                中断
            结束
        否则
            如果 循环异界地图() == 真 那么
                如果 根据类型获得物品数量(物品类型.异界地图) <= 0 那么
                    杀到指定地图("The Sarn Encampment",3,假)
                否则
                    如果 开启地图装置() ~= 真 那么
                        LogE("开启异界装置失败")
                        中断
