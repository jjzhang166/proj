...
--物品颜色: 白色 -> 蓝色 -> 金色 -> 暗金 
--过滤类型: 白名单、 黑名单、 颜色、 物品类型、 品质、 物品数量、 三色连、 宝石槽连接、 宝石槽连接、 绑定状态、 属性、装备、等级、失效宝石;  
...
--在白名单的存仓, 在黑名单的不存  
清除存仓过滤()
添加存仓过滤(创建过滤(过滤类型.白名单,{"Albino Rhoa Feather","Alchemy Shard","Alteration Shard","Transmutation Shard","Scroll Fragment","Blessed Orb","Divine Orb","Orb of Augmentation","Exalted Orb","Orb of Scouring","Orb of Alteration","Orb of Transmutation","Orb of Chance","Orb of Alchemy","Mirror of Kalandra","Chaos Orb","Orb of Regret","Silver Coin","Perandus Coin","Eternal Orb","Enchant","Prophecy","Blacksmith's Whetstone","Armourer's Scrap","Glassblower's Bauble","Gemcutter's Prism","Cartographer's Chisel","Chromatic Orb","Orb of Fusing","Jeweller's Orb"}))
...
添加存仓过滤(创建过滤(过滤类型.物品类型,{物品类型.地图碎片,物品类型.命运卡,物品类型.异界地图,物品类型.珠宝}))
...
--憎恨
添加技能(创建技能("Hatred"):释放间隔(4):技能CD(0.05):直接施放():优先级(0))
--纪律
添加技能(创建技能("Discipline"):释放间隔(3):技能CD(0.05):直接施放():优先级(0))
--召唤愤怒狂灵
添加技能(创建技能("Summon Raging Spirit"):释放间隔(0):技能CD(3):血值区间(0.1, 1):蓝值区间(0.1, 1):施法距离(0.3, 0.35):优先级(10):边逃边杀()
    :增加前置技能({"Summon Flame Golem","Raise Zombie", "Flesh Offering","Hatred"})
)
--电能释放
...
--召唤巨石魔像
添加技能(创建技能("Summon Stone Golem"):释放间隔(3):技能CD(0.05):施法距离(0.3, 0.5):优先级(2000):边逃边杀())
----清晰
--添加技能(创建技能("Clarity"):释放间隔(3):技能CD(0.05):直接施放():优先级(0))
...
--电能释放
设置宝石连接(0,10,"Lightning Tendrils",{})
--火球
设置宝石连接(0,10,"Fireball",{})
--憎恨--和善
设置宝石连接(24,100,"Hatred",{"Generosity Support"})
--清晰
设置宝石连接(0,100,"Clarity",{})
--纪律
设置宝石连接(0,100,"Discipline",{})
--唤醒幽魂
设置宝石连接(0,100,"Raise Spectre",{})
--召唤巨石魔像
设置宝石连接(0,100,"Summon Stone Golem",{})
