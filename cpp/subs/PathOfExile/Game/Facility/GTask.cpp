...
bool GCurQuestInfo::UpdateImpl()
{
	...
	if (player_me_->FindQuestState(quest_state, ui_quest_item_->db_quest_
#ifndef VER3_
		, ui_map->difficulty_
#endif
		) != &quest_state)
...
bool GUiQuestStateInfo::UpdateImpl()
{
	...
	CD_Interface::TraUiQuestStateItem([this](stCD_UiQuestStateQuestItem* item){
		...
		if (WideCharToGbk(buffer_quest) == kTaskName_Labyrinth)
			return true;
		quest_state_cont_.push_back(stQuestState());
		auto& back = quest_state_cont_.back();
		back.quest_name_ = WideCharToGbk(buffer_quest);
		auto states = WideCharToGbk(buffer_state);
		assert(!states.empty());
		states += "\n";
		while (true)
		{
			auto pos = states.find("\n");
			if (pos == std::string::npos || pos + 1 == std::string::npos)
				break;
			back.states_.push_back(std::string(states.begin(), states.begin() + pos));
			states.erase(states.begin(), states.begin() + pos + 1);
		}
	...
	if (db_quest.IsBranchLine())
	{
		for (auto& v : quest_state_cont_)
		{
			if (v.quest_name_.find(quest_name) != 0)
				continue;
			auto it = std::find_if(v.states_.begin(), v.states_.end(), [&quest_state_name](const std::string& str){return str == quest_state_name; });
			if (it == v.states_.end())
				continue;
