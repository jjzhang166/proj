...
class GItemTipsPanel{
	typedef std::vector<CSV_T<CSV_Catcher> > CalculatorCont;
	typedef std::map< CSV_T<CSV_Catcher>, PtValue>	PropRec;
public:
	GItemTipsPanel() : parse_finish_(false){
		check_sum_ = 0;
	}
	virtual bool From(stCD_ItemTipsParams* game_tips);
	virtual bool ParseProp() const;
	virtual const PropRec& GetProp() const;
...
public:
	...
	void DebugInfo() const;
	pt_dword Checksum() const{ return check_sum_; }

protected:
	static CalculatorCont			calculator_;
	GItemAttrStringCont				rows_;
	mutable PropRec					item_prop_;
	mutable bool					parse_finish_;
	pt_dword						check_sum_;
};

class GItemBaseProp : public GItemTipsPanel{
public:
	bool From(stCD_ItemTipsParams* game_tips) override;
};

class GItemSubjoinProp : public GItemTipsPanel{
public:
	bool From(stCD_ItemTipsParams* game_tips) override;
};

struct GItemAttrFormatter{
	typedef std::vector<std::string> StrCont;

	bool From(const GameWString& format_str);
...
public:
	...
	stCD_EntityItemBased* GetItemEntity() const{ return item_entity_; }
	...
	inline pt_dword GetItemLvl() const{ return item_level_; }
	inline const stCD_ItemGridsIdxInfo& GetItemGridInfo() const{ return item_idx_info_; }
	inline const GItemBaseProp& GetItemBaseProp() const { return item_base_prop_; }
	inline const GItemSubjoinProp& GetItemSubjoinProp() const { return item_subjoin_prop_; }
	inline bool IsWeapon() const{ return item_cate_ == kICATE_Weapon; }
	inline bool IsRing() const { return item_cate_ == kICATE_Ring; }
	inline bool IsGem() const { return item_cate_ == kICATE_Gem; }
	inline bool IsFlask() const{ return item_cate_ == kICATE_Flask; }
	inline enItemCate GetCateType() const { return item_cate_; }
	inline bool IsQuestItem() const{ return item_cate_ == kICATE_Quest; }
	inline const SellPriceCont& GetPrice() const{ return price_; }
...
public:
	...
	pt_dword GetQuality() const;
	stCD_VecInt GetCurItemPos() const;
	bool CanWear() const;					//是否满足佩戴需求
	int Weight() const;
	virtual GChecksumType CheckSum(int bag_width) const;
	GChecksumType SimpleChecksum() const;
	luabind::object GroupPropTable(lua_State* lua_l, enGroupPropflag flag = kGPF_AllProp) const;
...
public:
	...
	virtual void GetGemIndexByName(const std::string& gem_name, GVectorInt& ret_index){ return; }
	virtual enCD_GemSlotColor GetColorByIndex(int index) const { return kGSC_Invalid; }
	...
	virtual void PrintWeight() const;
...
protected:
	...
	GChecksumType GetChecksumImpl() const;
...
private:
	...
	GItemBaseProp			item_base_prop_;
	GItemSubjoinProp		item_subjoin_prop_;
	mutable GItemWeightType	weight_;
	std::wstring			tips_format_str_;
...
public:
	...
	void GetGemIndexByName(const std::string& gem_name, GVectorInt& ret_index) override;
	enCD_GemSlotColor GetColorByIndex(int index) const override;
	...
	GChecksumType CheckSum(int bag_width) const override;
...
public:
	...
	GItemBasePtr GetAnyCanWearEquip(const GWearGoodEquips& good_equips, enCD_InventoryType dst_inventory_type) const;
