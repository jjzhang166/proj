...
bool GItemBase::UpdateImpl()
{
	...
	item_level_ = tips->item_lvl_;
	if (!item_base_prop_.From(tips))
	{
		assert(false);
		return false;
	}
	if (!item_base_prop_.ParseProp())
	{
		assert(false);
		return false;
	}
	if (!item_subjoin_prop_.From(tips))
	{
		assert(false);
		return false;
	}
	auto format_str = ui_tips->GenAttrFormatStr(item_entity_);
	if (!format_str)
	{
		assert(false);
		return false;
	}
	tips_format_str_ = format_str->c_str();
	assert(!tips_format_str_.empty());
	return true;
}
...
void GItemBase::DebugInfo() const
{
	...
	LOG_O(Log_debug) << "item-name :" << GetItemName() << ",item_lvl:" << item_level_ << ",item_id :" << GetItemId()
		...
		<< ",物品类型:" << GetItemType();
	item_base_prop_.DebugInfo();
	item_subjoin_prop_.DebugInfo();
}
...
{
	return GEquipScore::GetMe().CalcItemGoodValue(*this);
}
...
GChecksumType GItemBase::CheckSum(int bag_width) const
{
	GChecksumType check_sum = 0;
	int cursor = 0;
	pt_dword value_tmp = 0;
	while (true)
	{
...
void GItemBase::PrintWeight() const
{
	GEquipScore::GetMe().PrintItemWeightInfo(*this);
}

luabind::object GItemBase::GroupPropTable(lua_State* lua_l, enGroupPropflag flag) const
{
	...
	tbl[kLuaKeyNameItemType] = item_type_;
	tbl[kLuaKeyItemLevel] = item_level_;
	object tbl_prop = newtable(lua_l);
	if (flag & kGPF_BaseProp)
	{
		auto& base_prop_info = item_base_prop_.GetProp();
		for (auto& i : base_prop_info)
		{
			if (!i.first || !i.second)
				continue;
	...
	if (flag & kGPF_SubjoinProp)
	{
		auto& subjoin_prop_info = item_subjoin_prop_.GetProp();
		for (auto& v : subjoin_prop_info)
		{
			if (!v.first || !v.second)
				continue;
			tbl_prop[v.first->GetName()] = v.second->GetValue(lua_l);
		}
	}
...
pt_dword GItemBase::GetQuality() const
{
	auto& base_prop = item_base_prop_.GetProp();
	if (base_prop.empty())
		return 0;
	for (auto& i : base_prop)
	{
		if (i.first->GetName() != kPropQualityName)
			continue;
		auto& values = i.second->GetValues();
		if (values.empty())
			break;
		return values[0]->GetValue(0);
	}
	return 0;
}

GChecksumType GItemBase::GetChecksumImpl() const
{
	GChecksumType check_sum = 0, check_sum_tmp = 0;
	check_sum_tmp = (pt_dword)item_cnt_;
	check_sum_tmp <<= kIIC_ItemCnt;
	check_sum += check_sum_tmp;

	pt_dword value_tmp = 0;
	for (auto c : item_name_)
		value_tmp += (pt_dword)c;

	check_sum_tmp = value_tmp;
	check_sum_tmp <<= kIIC_ItemName;
	check_sum += check_sum_tmp;

	check_sum_tmp = item_id_;
	check_sum_tmp <<= kIIC_ItemId;
	check_sum += check_sum_tmp;

	check_sum_tmp = (pt_dword)Weight();
	check_sum_tmp <<= kIIC_Weight;
	check_sum += check_sum_tmp;

	check_sum_tmp = item_inventory_type_;
	check_sum_tmp <<= kIIC_ItemInventoryType;
	check_sum += check_sum_tmp;

	check_sum_tmp = astrict_.Checksum();
	check_sum_tmp <<= kIIC_BasePro;
	check_sum += check_sum_tmp;

	check_sum_tmp = max_stack_cnt_ + item_level_ + item_color_ * 4 + item_type_ * 10 + item_cate_ * 20;
	check_sum_tmp += (pt_dword)(item_size_.x * item_size_.y);
	check_sum_tmp += is_identified_ * 24 + is_bind_ * 26;
	check_sum_tmp <<= kIIC_ItemValues;
	check_sum += check_sum_tmp;
	...
	for (auto& v : price_)
	{
		...
		{
			for (auto c : v.attr_db_item_base_->name_)
				value_tmp += (pt_dword)c;
		}
		else
			assert(false);
		++value_tmp;
		value_tmp *= v.item_cnt_;
		check_sum_tmp += value_tmp;
	}
	check_sum_tmp <<= kIIC_Price;
	check_sum += check_sum_tmp;

	check_sum_tmp = item_base_prop_.Checksum() + item_subjoin_prop_.Checksum();
	check_sum_tmp <<= kIIC_ItemTips;
	check_sum += check_sum_tmp;

	value_tmp = 0;
	for (auto c : tips_format_str_)
	{
		value_tmp += (pt_dword)c;
	}
	check_sum_tmp = value_tmp;
	check_sum_tmp <<= kIIC_TipsFormatStr;
	check_sum += check_sum_tmp;

	return check_sum;
}

GChecksumType GItemBase::SimpleChecksum() const
{
	assert(false);
	return 0;
}
...
void GItemEquipBase::GetGemIndexByName(const std::string& gem_name, GVectorInt& ret_index)
{
	if (gem_solt_.empty())
		return;
	for (auto& i : gem_solt_)
...
enCD_GemSlotColor GItemEquipBase::GetColorByIndex(int index) const
{
...
void GItemEquipBase::PrintWeight() const
{
	GEquipScore::GetMe().PrintItemWeightInfo(*this);
	__super::PrintWeight();
}
...
GChecksumType GItemEquipBase::CheckSum(int bag_width) const
{
	auto check_sum = __super::CheckSum(bag_width);
	for (auto& i : gem_solt_)
	{
		check_sum += i.Checksum();
	}
	return check_sum;
}
...
GChecksumType GPackageBase::CheckSum() const
{
	GChecksumType check_sum = 0, checksum_tmp = 0;
	for (auto& item : items_)
	{
		check_sum += item->CheckSum(package_size_.x);
	}
	checksum_tmp = package_type_;
	checksum_tmp <<= kIIC_InventoryType;
	check_sum += checksum_tmp;
	return check_sum;
}
...
pt_dword GPackageBase::GetItemCntByName(const std::string& item_name) const
{
	if (items_.empty())
		return 0;
	pt_dword cnt = 0;
	...
	for (auto& i : items_)
	{
		...
		gem_vec.clear();
		i->GetAllPackGem(gem_vec);
		if (gem_vec.empty())
			continue;
		for (auto& v : gem_vec)
		{
			if (v->GetItemName() == item_name)
				cnt += v->GetItemCnt();
		}
	}
	return cnt;
...
GItemBasePtr GPackageBase::GetAnyCanWearEquip(const GWearGoodEquips& good_equips, enCD_InventoryType dst_inventory_type) const
{
	auto good_it_begin = good_equips.begin(), good_it_end = good_equips.end();
	auto& type_mgr = GEquipTypeMgr::GetMe();
	for (auto& item : items_)
	{
		auto good_it = std::find_if(good_it_begin, good_it_end, [&item](const GWearEquipInfo& equip_info){
			if (item == equip_info.equip_)
				return true;
			return false;
		});
		if (good_it != good_it_end)
			continue;
		...
		for (auto position : package_positions)
		{
			...
			if (position != dst_inventory_type)
				continue;
			if (item->GetCateType() != kICATE_Weapon)
				return item;
			assert(!good_equips[position - 2].equip_);
			auto other_pos = type_mgr.SwitchPosition(position);
			if (other_pos == kIT_Invalid)
			{
				assert(false);
				continue;
			}
			if (good_equips[other_pos - 2].equip_)
				return nullptr;
			return item;
		}
	}
...
void GItemTipsPanel::DebugInfo() const
{
	for (auto& v : rows_)
		LOG_O(Log_trace) << v;
	if (!item_prop_.empty())
		LOG_O(Log_trace) << "解析后的属性信息-----------------";
	for (auto& i : item_prop_)
		LOG_O(Log_trace)<< i.first->GetName();
}
...
CSV_NodePtr GItemTipsPanel::CreateItemAttrParser(const std::string& pro_name)
{
	auto res = CSV_Factory::MakeCatcher(pro_name);
	if (!res)
		return nullptr;
	for (auto& i : calculator_)
	{
		if (i->GetName() != pro_name)
			continue;
		LOG_O(Log_debug) << "出现重复定义属性名:" << pro_name;
		assert(false);
		return nullptr;
	}
...
bool GItemTipsPanel::ParseProp() const
{
...
const GItemTipsPanel::PropRec& GItemTipsPanel::GetProp() const
{
	if (!parse_finish_)
		ParseProp();
	return item_prop_;
...
bool GItemBaseProp::From(stCD_ItemTipsParams* game_tips)
{
	assert(game_tips);
	pt_dword checksum_tmp = 0;
	if (!game_tips->item_words_.empty())
	{
		for (auto c : game_tips->item_words_)
		{
			checksum_tmp += (pt_dword)c;
		}
		check_sum_ += checksum_tmp;
...
bool GItemSubjoinProp::From(stCD_ItemTipsParams* game_tips)
{
	assert(game_tips);
	pt_dword checksum_tmp = 0;
	if (!game_tips->item_words_.empty())
...
bool GItemAttrFormatter::From(const GameWString& format_str)
{
	boost::char_separator<wchar_t> sep(L"\r\n");
	typedef boost::tokenizer<boost::char_separator<wchar_t>, GameWString::const_iterator, std::wstring>
		CustomTokenizer;
	CustomTokenizer tok(format_str.begin(), format_str.end(), sep);
	...
	for (CustomTokenizer::iterator beg = tok.begin(); beg != tok.end(); ++beg)
	{
		...
		if (str_tmp.empty())
			...
			continue;
		attrs_.push_back(WideCharToGbk(*beg));
	}
	return true;
}
