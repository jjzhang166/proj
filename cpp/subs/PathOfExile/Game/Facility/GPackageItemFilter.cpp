...
bool FilterItemQualityByRegion::DoFilt(const GItemBase& item) const
{
	auto item_quality = item.GetQuality();
	return item_quality >= min_quality_ && item_quality <= max_quality_;
}

bool FilterItemQualityByRegion::Parse(const luabind::object& lua_obj)
{
	using namespace luabind;
	...
	min_quality_ = LuaObjCast(lua_obj[1], INT_MAX);
	max_quality_ = LuaObjCast(lua_obj[2], INT_MAX);
	return true;
}
...
bool FilterItemBind::DoFilt(const GItemBase& item) const
{
	return item.IsBind() == bind_state_;
}
...
GFilterItemPtr GItemFilterFactory::MakeItemFilter(enItemFilterType filter_type)
{
	...
	case KIFT_Emboitement:
		...
		break;
	case kIFT_ItemProp:
		filter.reset(new FilterItemProp());
		break;
	case kIFT_Equip:
		filter.reset(new FilterItemEquipType());
		break;
	case kIFT_Quality:
		filter.reset(new FilterItemQualityByRegion());
		break;
	case kIFT_Level:
		filter.reset(new FilterItemLevel());
		break;
	case KIFT_InvalidGem:
		filter.reset(new FilterInvalidGem());
		break;
	default:
		break;
	}
...
bool FilterItemPropValue::DoFilt(const GItemBase& obj) const
{
	using namespace luabind;
	auto& base_prop = obj.GetItemBaseProp();
	auto& base_prop_info = base_prop.GetProp();
	if (base_prop_info.empty())
		return true;
	if (!fn_.is_valid())
	{
		assert(false);
	...
	auto lua_l = fn_.interpreter();
	if (!lua_l)
	{
		assert(false);
		return true;
	}
	auto tbl = obj.GroupPropTable(lua_l);
	if (type(tbl) != LUA_TTABLE)
	{
		assert(false);
		return true;
	}
	return LuaObjCallByDlt(fn_, false, tbl);
}
...
{
	auto it = std::find_if(lock_item_.begin(), lock_item_.end(), [obj](const GClass::stEmboitementInfo& tmp){return tmp.item_color_ == obj.GetItemColor() && tmp.item_type == GInventoryMgr::ConvertEmboitementType(obj.GetItemType()); });
...
bool FilterItemEmboitement::Parse(const luabind::object& lua_obj)
{
	...
	if (lua_obj.is_valid())
	{
		...
		check_cnt_ = LuaObjCast(lua_obj[1], true);
	}
	return true;
}

FilterItemEmboitement::FilterItemEmboitement(GClass::LackConT& loack_item_) : lock_item_(loack_item_), check_cnt_(true)
{
	GInventoryMgr::InitEmboitement();
}

bool FilterItemProp::Parse(const luabind::object& lua_obj)
{
	using namespace luabind;
	if (type(lua_obj) != LUA_TFUNCTION)
	{
...
bool FilterItemProp::DoFilt(const GItemBase& obj) const
{
	using namespace luabind;
	auto& base_prop = obj.GetItemBaseProp();
	auto& base_prop_info = base_prop.GetProp();
...
bool FilterItemEquipType::DoFilt(const GItemBase& item) const
{
	return item.IsEquip();
}

bool FilterItemEquipType::Parse(const luabind::object& lua_obj)
{
	return true;
}

bool FilterItemQualityRegion::DoFilt(const GItemBase& obj) const
{
	auto quality = obj.GetQuality();
	return quality >= min_quality_ && quality <= max_quality_;
}

bool FilterInvalidGem::DoFilt(const GItemBase& item) const
{
	auto& auto_Gem = GAutoUpDateGem::GetMe();
	auto link_info = auto_Gem.FindGemGroup(item.GetItemName());
	if (!link_info)
		return true;
	auto role_lvl = GPlayerMe::GetMe().GetRoleLvl();
	if (role_lvl > link_info->max_lvl_)
		return true;
	return false;
}

bool FilterInvalidGem::Parse(const luabind::object& lua_obj)
{
	return true;
}

bool FilterInvalidGem::Before(const GItemBase& obj) const
{
	return obj.IsGem();
}

bool FilterItemLevel::Parse(const luabind::object& lua_obj)
{
	using namespace luabind;
	if (type(lua_obj) != LUA_TTABLE)
	{
	...
	min_level_ = LuaObjCast(lua_obj[1], INT_MAX);
	max_level_ = LuaObjCast(lua_obj[2], INT_MAX);
	return true;
}

bool FilterItemLevel::DoFilt(const GItemBase& obj) const
{
	auto item_lvl = obj.GetItemLvl();
	return item_lvl >= min_level_ && item_lvl <= max_level_;
}
