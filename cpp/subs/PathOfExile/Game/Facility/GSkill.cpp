...
bool GBuffMgr::UpdateImpl()
{
	...
	if (!GWndExecSync([this](){
		...
		for (auto buff : life->buff_cont_)
		{
			...
			auto tmp_str = WideCharToGbk(CD_SafeName(buff->GetBuffCnName()));
			if (tmp_str.empty() || tmp_str == kBuffName_GracePeriod || tmp_str == kBuffName_SummonRagingSpirit)
				continue;
			buff_items_.push_back(stBuffItem());
			buff_items_.back().buff_name_ = tmp_str;
		}
		CD_Interface::TraCallerBuff([this](stCD_SkillInfo* skill_info, stCD_UiBuffItem* ui_item){
			...
			if (!active_skill)
				return true;
			auto buff_name = WideCharToGbk(CD_SafeName(active_skill->ch_name_));
			if (buff_name == kBuffName_SummonRagingSpirit)
				return true;
			buff_items_.push_back(stBuffItem());
			auto& the_item = buff_items_.back();
			the_item.buff_name_ = buff_name;
