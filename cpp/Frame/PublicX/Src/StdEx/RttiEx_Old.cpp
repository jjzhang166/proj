...
#define CHD_MULTINH 0x00000001		//表示多继承。
#define CHD_VIRTINH 0x00000002		//存在虚拟继承。
#define CHD_AMBIGUOUS 0x00000004	//继承情况模糊不清。
#define BCD_NOTVISIBLE 0x00000001	//一般private和protected继承时设置此位。
#define BCD_AMBIGUOUS 0x00000002	//与CHD_AMBIGUOUS一样。
#define BCD_PRIVORPROTINCOMPOBJ 0x00000004	//一般private和protected继承时设置此位。
#define BCD_PRIVORPROTBASE 0x00000008	//一般private和protected继承时设置此位。
#define BCD_VBOFCONTOBJ 0x00000010	//为虚基类。
#define BCD_NONPOLYMORPHIC 0x00000020	//非多态。
#define CHD_NUMBASES(chd) ((chd).numBaseClasses)
#define TYPEIDS_EQ(lhs, rhs) (&(lhs) == &(rhs))
#define BCD_ATTRIBUTES(bcd) ((bcd).attributes)
#define BCD_NUMCONTBASES(bcd) ((bcd).numContainedBases)
#endif
using namespace StdEx;
typedef RttiEx::RTTIBaseClassDescriptor _RTTIBaseClassDescriptor;
typedef RttiEx::RTTICompleteObjectLocator _RTTICompleteObjectLocator;
typedef RttiEx::TypeDescriptor _RTTITypeDescriptor;
typedef RttiEx::PMD PMD;
typedef RttiEx::RTTIBaseClassArray _RTTIBaseClassArray;
...
PVOID RttiEx::__RTCastToVoid(PVOID inptr)            // Pointer to polymorphic object
{
	if (inptr == NULL)
		return NULL;
...
PVOID RttiEx::__RTtypeid(PVOID inptr)            // Pointer to polymorphic object
{
	if (!inptr) {
#ifndef _SYSCRT
	...
	__try {
		...
		if (((const void *)COL_PTD(*pCompleteLocator)) != NULL)
		{
	...
	)
{
	...
	_RTTIBaseClassDescriptor *pTargetBCD = NULL;
	_RTTIBaseClassDescriptor *pSourceBCD = NULL;
...
{
	_RTTIBaseClassDescriptor *pBCD;
	_RTTIBaseClassDescriptor *pDownCastResultBCD = NULL;
	_RTTIBaseClassDescriptor *pCrossCastSourceBCD = NULL;
	_RTTIBaseClassDescriptor *pCrossCastTargetBCD = NULL;
	_RTTIBaseClassDescriptor *pTargetBCD = NULL;
	_RTTIBaseClassDescriptor *pSourceInTargetBCD;
	_RTTIBaseClassArray *pBaseClassArray = CHD_PBCA(*COL_PCHD(*pCOLocator));
	_RTTIBaseClassArray *pTargetBaseClassArray;
	...
	if (fDownCastAllowed && pDownCastResultBCD != NULL)
	{
	...
	if (pCrossCastSourceBCD != NULL && pCrossCastTargetBCD != NULL)
	{
