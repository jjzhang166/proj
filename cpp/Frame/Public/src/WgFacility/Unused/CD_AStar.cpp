...
const CD_AStar::stPosNode* CD_AStar::GenFindPathImpl(const stCD_VecInt& pos_src, const stCD_VecInt& pos_dst)
{
	open_.Clear();
	close_.Clear();
	...
	while (open_.nodes_._Myfirst != open_.nodes_._Mylast)
	{
		...
		for (auto& dir_info : dir_array)
		{
			...
			pos_tmp.y = cur_y;
			auto idx_tmp = pos_tmp.y * width_;
			for (; cur_x != end_x && cur_x < width_; cur_x += dir_info.x)
			{
				pos_tmp.x = cur_x + dir_info.x;
				auto idx = idx_tmp + pos_tmp.x;
			...
			pos_tmp.x = cur_x;
			for (; cur_y != end_y && cur_y < height_; cur_y += dir_info.y)
			{
				pos_tmp.y = cur_y + dir_info.y;
				auto idx = pos_tmp.y * width_ + pos_tmp.x;
				if (!pos_info_.test(idx))
					break;
				for (auto it = close_it; it != close_it_end; ++it)
				{
...
{
	auto pos_src_tmp = pos_src;
	pos_src_tmp *= zoom_rate_;
	if (!RectifyPos(pos_src_tmp, pos_src))
	{
		assert(false);
		return false;
	}
	
	auto pos_dst_tmp = pos_dst;
	pos_dst_tmp *= zoom_rate_;
	if (!RectifyPos(pos_dst_tmp, pos_dst))
	{
		assert(false);
		return false;
	}
	auto node = GenFindPathImpl(pos_src_tmp, pos_dst_tmp);
	if (!node)
		return false;
	...
	for (auto& pos : out_pos_info)
	{
		pos.x = int((float)pos.x / zoom_rate_);
		pos.y = int((float)pos.y / zoom_rate_);
	}
...
bool CD_AStar::Init(const stCD_MapData& map_data)
{
	//pos_info_.reset();
	auto map_size = map_data.GetMapSize();
	assert(map_size.x > 50 && map_size.y > 50);
	zoom_rate_ = CD_kAStar_SideSize / (float)map_size.x;
	zoom_rate_ = std::min(CD_kAStar_SideSize / (float)map_size.y, zoom_rate_);
	zoom_rate_ = zoom_rate_ * CD_kAStar_ZoomRate;
	width_ = (int)((float)map_size.x * zoom_rate_);
	assert(width_ > 0);
	height_ = (int)((float)map_size.y * zoom_rate_);
	assert(height_ > 0);
	auto max_array = width_ * height_;
	assert(max_array < CD_kAStar_SideSize * CD_kAStar_SideSize);
	auto get_dst_map_data = stCD_MapData::GetFn_GetDstMapData();
	stCD_VecInt pos_tmp;
	for (int i = 0; i != max_array; ++i)
	{
		pos_tmp.x = int(i % width_ / zoom_rate_);
		pos_tmp.y = int(i / width_ / zoom_rate_);
		if ((map_data.*get_dst_map_data)(pos_tmp, 0) <= enCD_MapDataFlag::kMountain)
		{
			pos_info_.reset(i);
			continue;
...
void CD_AStar::DebugPosInfo(const std::string& file_name) const
{
...
bool CD_AStar::RectifyPos(stCD_VecInt& pos, const stCD_VecInt& pos_src) const
{
	...
	if (pos_info_.test(pos.y * width_ + pos.x))
		return true;	
	LOG_O(Log_debug) << "该压缩后的坐标其实是阻碍点，压缩坐标：" << pos << "，原始坐标：" << pos_src;

	auto pos_tmp = pos;
	pos_tmp.x -= CD_kAStar_RectifiedSize;
	if (pos_tmp.x < 0)
		pos_tmp.x = 0;
	pos_tmp.y -= CD_kAStar_RectifiedSize;
	if (pos_tmp.y < 0)
		pos_tmp.y = 0;

	stCD_VecInt last_valid;
	float distance = -1;
	auto end_x = pos_tmp.x + CD_kAStar_RectifiedSize * 2;
	auto end_y = pos_tmp.y + CD_kAStar_RectifiedSize * 2;
	auto the_y = pos_tmp.y;
	for (auto x = pos_tmp.x; x < width_ && x < end_x; ++x)
	{
		for (auto y = the_y; y < height_ && y < end_y; ++y)
		{
			if (!pos_info_.test(y * width_ + x))
				continue;
			pos_tmp.x = x;
			pos_tmp.y = y;
			auto d = pos_tmp.Distance(pos);
