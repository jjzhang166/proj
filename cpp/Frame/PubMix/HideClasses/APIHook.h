...
class CAPIHook : public IHooker{
public:
   // Hook a function in all modules but this module
   CAPIHook(PCSTR pszCalleeModName, PCSTR pszFuncName, PROC pfnHook);
   // Hook a function in one module
   CAPIHook(PCSTR pszCalleeModName, PCSTR pszFuncName, PROC pfnHook, HMODULE hHookedModule);
...
private:
	//初始化
	bool Init( PCSTR pszCalleeModName, PCSTR pszFuncName, PROC pfnHook );
...
private:
   ...
   HMODULE	m_hHookedModule;	 // 被hook住的模块，若是为0，则表示hook住了所有模块
   bool		m_IsResume;			//在析构的时候是否恢复
