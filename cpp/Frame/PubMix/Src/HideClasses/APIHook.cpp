...
CAPIHook::CAPIHook(PCSTR pszCalleeModName, PCSTR pszFuncName, PROC pfnHook) {
   ...
   if ( !Init(pszCalleeModName, pszFuncName, pfnHook) )
   {
	   assert(false);
	   return;
   }
				   ...
				   HMODULE hHookedModule )
{
	...
	m_hHookedModule = hHookedModule;
	if ( nullptr == hHookedModule )
		 ReplaceIATEntryInAllMods(m_pszCalleeModName, m_pfnOrig, m_pfnHook);
	else
		ReplaceIATEntryInOneMod(m_pszCalleeModName, m_pfnOrig, m_pfnHook, m_hHookedModule);
}
...
CAPIHook::~CAPIHook() {
   ...
   // Unhook this function from all modules
	if (m_IsResume)
	{
		if ( nullptr == m_hHookedModule )
			ReplaceIATEntryInAllMods(m_pszCalleeModName, m_pfnHook, m_pfnOrig);
		else
			ReplaceIATEntryInOneMod(m_pszCalleeModName, m_pfnHook, m_pfnOrig, m_hHookedModule);
	}
...
FARPROC CAPIHook::GetProcAddressRaw(HMODULE hmod, PCSTR pszProcName) {
   ...
   return(LibFuncLoader::GetTheProcAddress(hmod, pszProcName));
}
...
void CAPIHook::ReplaceIATEntryInAllMods(PCSTR pszCalleeModName, 
   PROC pfnCurrent, PROC pfnNew) {
	...
	assert(!"尽量不要用API hook，用之前先考虑下游戏hook的crc校验问题，CmnX_AntiHookCheck");
   ...
   for (; pImportDesc->Name; pImportDesc++) {
      ...
      if (lstrcmpiA(pszModName, pszCalleeModName) == 0) {
         ...
         for (; pThunk->u1.Function; pThunk++) {
            ...
            if (bFound) {
               if ( !GetWriteProcessMemory()(GetCurrentProcess(), ppfn, &pfnNew, 
                 ...
                 DWORD dwOldProtect;
                  ...
                  if (VirtualProtect(ppfn, sizeof(pfnNew), PAGE_WRITECOPY, 
                     ...
                     GetWriteProcessMemory()(GetCurrentProcess(), ppfn, &pfnNew, 
                        sizeof(pfnNew), NULL);
...
HMODULE WINAPI CAPIHook::LoadLibraryA(PCSTR pszModulePath) {
   ...
   HMODULE hmod = LibraryLoader::LoadTheLibrary(pszModulePath);
...
bool CAPIHook::Init( PCSTR pszCalleeModName, PCSTR pszFuncName, PROC pfnHook )
{
...
PROC CAPIHook::GetApiOrig()
{
	return m_pfnOrig;
}

bool CAPIHook::IsValid()
{
	return !!m_pfnOrig;
}

void CAPIHook::SetResume( bool is_resume )
{
	m_IsResume = is_resume;
}
