...
# endif
  ...
  {
	  class SaveCallImpl : public SaveCall{
		  int DoCall() const override{
			  return InvokeImpl(impl_, ctx_, l_);
		  }

	  public:
		  SaveCallImpl(const function_object_impl* impl, invoke_context& ctx, lua_State* l)
			: impl_(impl), ctx_(ctx), l_(l){
		  }

	  private:
		  const function_object_impl*	impl_;
		  invoke_context&				ctx_;
		  lua_State*					l_;
	  };
	  ...
	  static int InvokeImpl( const function_object_impl* impl, invoke_context& ctx, lua_State* L ){
		  return invoke(L, *impl, ctx, impl->f, Signature(), impl->policies);
	  }
...
# ifndef LUABIND_NO_EXCEPTIONS
          ...
          try
          {
			  if (IsPreTryCatchValid())
			  {
				  SaveCallImpl save_call(impl, ctx, L);
				  results = DoPreTryCatch(save_call);
			  }
			  else
				  results = InvokeImpl(impl, ctx, L);
          }
          catch (...)
          {
