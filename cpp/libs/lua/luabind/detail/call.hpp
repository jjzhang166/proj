...
struct LUABIND_API invoke_context
{
    ...
    int best_score;
	function_object const* candidates[LUABIND_MAX_ARITY];
    int candidate_index;
};
...
//by zhp, 2017/1/15 16:48
template<typename Policy>
inline bool IsRawCall_(bool arity_greater, Policy*){
	return false;
}
...
# endif
	...
	if (arity == arguments || IsRawCall_(arguments > arity, (Policies*)0))
    {
        int const scores[] = {
            0
# if N > 0
...
# endif
    ...
    {
		assert(ctx.candidate_index < _countof(ctx.candidates) - 1);
