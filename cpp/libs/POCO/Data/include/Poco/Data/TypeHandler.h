...
/*
准备修改TypeHandler来扩展功能。
比如支持枚举类型。
2016/5/8 22:37
*/
struct TypeHandlerTag_Enum{};
template<typename T>
struct TypeHandlerTag{
	typedef typename std::conditional<std::is_enum<T>::value, TypeHandlerTag_Enum, void>::type type;
};
template<typename T, typename Tag_>
class TypeHandlerBase : public AbstractTypeHandler
...
template<typename T>
class TypeHandlerBase<T, TypeHandlerTag_Enum> : public AbstractTypeHandler{
public:
	...
	{
		poco_assert_dbg(!pBinder.isNull());
		auto ptr_ptr = (const int*)(const void*)(const T*)(&obj);
		pBinder->bind(pos, *ptr_ptr, dir);
	}
	...
	{
		poco_assert_dbg(!pExt.isNull());
		auto ptr_ptr = (int*)(void*)(T*)(&obj);
		if (!pExt->extract(pos, *ptr_ptr)) obj = defVal;
	}
	...
	static void prepare(std::size_t pos, const T& obj, AbstractPreparator::Ptr pPreparator)
	{
		poco_assert_dbg(!pPreparator.isNull());
		auto ptr_ptr = (const int*)(const void*)(const T*)(&obj);
		pPreparator->prepare(pos, *ptr_ptr);
	}
...
template<typename T>
class TypeHandler : public TypeHandlerBase<T, typename TypeHandlerTag<T>::type>{
};
